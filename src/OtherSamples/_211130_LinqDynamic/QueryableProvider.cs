...
namespace FreeSql.Extensions.Linq
{
    class QueryableProvider<TCurrent, TSource> : IQueryable<TCurrent>, IOrderedQueryable<TCurrent> where TSource : class
    {
        ...
        public Type ElementType => typeof(TCurrent);
    ...
    class QueryProvider<TCurrent, TSource> : IQueryProvider where TSource : class
    {
        ...
        public IQueryable CreateQuery(Expression expression)
        {
            ...
            if (elementType != typeof(TCurrent))
            {
                var queryProviderType = typeof(QueryProvider<,>).MakeGenericType(elementType,typeof(TSource));
                var queryProvider = Activator.CreateInstance(queryProviderType, _select, expression);
                var queryableProvider = typeof(QueryableProvider<,>).MakeGenericType(elementType, typeof(TSource));
                return Activator.CreateInstance(queryableProvider, expression, queryProvider, _select) as IQueryable;
            }
            _oldExpression = expression;
            return new QueryableProvider<TSource, TSource>(expression, this, _select);
        }
        ...
        {
            return ExecuteExp(expression, expression.Type, _oldExpression == expression);
        }
